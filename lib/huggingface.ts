...
export async function generateProfessionalSummary(profile: any, repositories: any[]): Promise<string> {
  try {
    // Using Hugging Face's free inference API
    // You can use models like: microsoft/DialoGPT-medium, gpt2, or facebook/blenderbot-400M-distill
...
function generateTemplateSummary(profile: any, repositories: any[]): string {
  ...
  const primaryLanguage =
    Object.entries(topLanguages).sort(([, a], [, b]) => (b as number) - (a as number))[0]?.[0] || "Software"

  const totalStars = repositories.reduce((sum, repo) => sum + repo.stargazers_count, 0)
  const yearsActive = Math.max(
    1,
    new Date().getFullYear() - new Date(repositories[0]?.created_at || "2020").getFullYear(),
  )
...
export async function generateProjectDescription(
  ...
  try {
    const prompt = `Write a professional project description for a ${language} project named "${projectName}" with topics: ${topics.join(", ")}. Description should be 1-2 sentences:`
...
function generateTemplateDescription(projectName: string, language: string, topics: string[]): string {
  const templates = [
